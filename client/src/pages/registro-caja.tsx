...
export default function RegistroCaja() {
  ...
  const { data: movimientos, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      // Usar la opción de emergency mode (user_id en la URL)
      const res = await apiRequest("GET", "/api/caja/movimientos?user_id=1");
      return res.json();
    }
  });
  ...
  const { data: resumenCaja, isLoading: isLoadingResumen } = useQuery({
    ...
    queryFn: async () => {
      // Usar la opción de emergency mode (user_id en la URL)
      const res = await apiRequest("GET", "/api/caja/resumen?user_id=1");
      return res.json();
    }
  });
  ...
  const eliminarMovimientoMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await apiRequest("DELETE", `/api/caja/movimientos/${id}?user_id=1`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Administra los ingresos y egresos de la caja
          </p>
        </div>
        <Button 
